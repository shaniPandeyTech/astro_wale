    ...
    fetch('https://api.indiandetectiveservices.com/public/api/v1/astrologers')
      ...
      .then(resData => {
        if (resData.success === 1 && Array.isArray(resData.data)) {
          setAstrologer(resData.data); // ✅ store only data array
        } else {
              ...
              {astrologer.map((item, index) => (
                <div key={index} className="bg-white rounded-xl border shadow-sm p-4 hover:shadow-md  relative cursor-pointer transform transition-transform hover:scale-105 duration-300  ">
                  ...
                  <div className="flex items-center space-x-2 text-sm mb-2">
                    ...
                    <span className="bg-gray-100 px-2 py-0.5 rounded text-xs text-gray-600">{item.experience} Years of Experience</span>
                  </div>
